...
### Added
- **Installable PWA with offline support.** A `manifest.webmanifest` (+ 192/512
  and maskable PNG icons generated from the brand mark, and an apple-touch-icon)
  makes Fora installable. A service worker — generated by a post-build
  `workbox-build` script (`web/scripts/build-sw.mjs`), independent of the bundler
  so it's immune to the Vite 8 / rolldown toolchain — **precaches the app shell**
  so the app loads and runs offline after one visit, and reloads/reopens work
  offline. Per-conference datasets (emitted under `assets/data/`) are
  **runtime-cached on first visit** (StaleWhileRevalidate) instead of precached,
  so the precache stays small and doesn't grow as conferences are added. The
  service worker never intercepts the Netlify OAuth broker (`navigateFallback`
  denylist), and registers in production only.
- **Instant resync on reconnect.** `@repus/gist-sync` now listens for the browser
  `online` event and syncs immediately when connectivity returns, instead of
  waiting out the exponential-backoff retry timer (previously up to ~5 min).
  Offline follow changes therefore push as soon as the network is back.
