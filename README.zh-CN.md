...
Fora 把密集的会议日程变得真正好用。挑选一个会议,浏览它的议程,通过给感兴趣的
内容加星来构建你的个人日程,并把它随身带到每一台设备上 —— 无需安装任何应用。

## 功能

- 🏛️ **多会议,一个站点** —— 挑选其一,直接进入完整日程。
- 🗓️ **完整的会议内容** —— 主旨报告、并行分会场与讲者,全部中英双语;时间视图把同时进行的会场并排呈现,并让你一眼看到此刻正在进行的内容。
- ⭐ **专属日程** —— 给在意的报告、会场和讲者加星,汇成一份可筛选、可回看的个人日程。
- 🔄 **多设备同步** —— 登录即可让日程在各设备间保持同步;仅浏览无需登录。
- 📲 **安装与离线** —— 添加到设备后,断网也能继续使用。
- 📤 **导出与备份** —— 把日程导入你的日历,或导出为可再次导入的备份。
- 🔍 **人物** —— 搜索讲者目录,浏览每个会议的委员会与组织机构。
